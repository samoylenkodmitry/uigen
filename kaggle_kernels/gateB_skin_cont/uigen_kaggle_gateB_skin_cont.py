"""V7 Gate B-skin continuation Kaggle training kernel.

Resumes from the 80k Gate B-skin checkpoint and runs another 80 000
optimizer steps at batch=14 (covering every skin every step) with
--within-file-replacement false. Recipe details live in
experiments/v7_completer_gateB_skin_continuation.yaml.

Expected runtime on Tesla T4 (from batchbench):
    ~173 min training + ~1 min eval = ~3 hr total.

Attached datasets:
    uigen-data                  -- the 16-skin completion dataset.
    uigen-gatebskin-80k-ckpt    -- last.safetensors of the 80k source run.

Snapshots every 10 000 steps so a session-time-out leaves recoverable
checkpoints.
"""
...
REPO = WORK / "uigen"
RUNS_DIR = WORK / "runs"
OUT = WORK / "gateB_skin_cont_outputs"
...
CKPT = Path(
    "/kaggle/input/datasets/dmitriisamoilenko/uigen-gatebskin-80k-ckpt/last.safetensors"
)
...
# Sanity check up front: the resume checkpoint must be where the YAML
# claims it is, otherwise the trainer crashes after the slow git clone.
...
# 3. Dry-run, then training. Streamed (capture=False) so we watch progress live.
summaries.append(run("05_kaggle_dry", [
    sys.executable, "scripts/run_experiment.py",
    "--experiment", "experiments/v7_completer_gateB_skin_continuation.yaml",
    "--dry-run",
...
# 4. Locate the run directory (YAML pins it).
run_dir = RUNS_DIR / "v7_completer_gateB_skin_continuation"
...
eval_json = OUT / "gateB_skin_cont_eval.json"
...
# 6. Persist artefacts: last + best + every snapshot + metrics + config + manifest.
import shutil
...
if eval_json.exists():
    ...
    print("\n=== Gate B continuation aggregate metrics ===", flush=True)
...
print("\nGATEB_SKIN_CONT_RESULT: TRAINED" if all(s["rc"] == 0 for s in summaries) else "GATEB_SKIN_CONT_RESULT: FAIL", flush=True)
