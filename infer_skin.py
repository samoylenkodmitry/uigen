...
def main() -> int:
    ...
    parser.add_argument("--image", default=None, help="reserved for GeoNet/SlotNet inference")
    parser.add_argument("--geonet", default=None)
    parser.add_argument("--slotnet", default=None)
    parser.add_argument("--atlas", default=None, help="export an already predicted/packed atlas")
    ...
    if args.atlas is None:
        raise SystemExit(
            "model inference is blocked until GeoNet/SlotNet checkpoints exist; "
            "use --atlas to export an existing packed atlas"
        )
