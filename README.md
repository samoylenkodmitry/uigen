...
## Build A Fresh V3.4 Dataset

Put raw skins in `skins_raw/` as folders or `.wsz` archives, then run:

```bash
python scripts/00_scan_skins.py --skins-raw skins_raw --out data_v34/skin_scan.csv
python scripts/01_pack_skins.py --skins-raw skins_raw --out data_v34
python scripts/02_render_dataset.py --valid-skins data_v34/valid_skins.csv --out data_v34 --variants 16 --cranamp-cli ./cranamp_cli/cranamp-cli
python scripts/03_make_splits.py --data data_v34 --valid-skins data_v34/valid_skins.csv
python scripts/04_check_dataset.py --data data_v34
```

The split CSVs contain only the V3.4 training contract: rendered input PNG,
...
python train_slotnet.py \
  --train data_v34/train.csv \
  --steps 20000 \
  --batch 2 \
  --base-channels 24 \
  --out runs/slotnet_v34_1skin \
  --device cuda
```

For a one-skin overfit smoke, generate a one-skin `data_v34` first and train
with `--limit-rows` if needed. Pre-V3.4 datasets, checkpoints, and eval folders
...
python infer_skin.py \
  ...
  --slotnet runs/slotnet_v34_1skin/best.safetensors \
  --out out_skin
```
